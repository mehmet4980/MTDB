...
@show
    ...
    <head>
        ...
        @section('assets')
            ...
            {{ HTML::style('themes/original/assets/css/styles.min.css?v13') }}
            {{ Hooks::renderCss() }}
        @show
    </head>
    ...
    {{ HTML::script('assets/js/scripts.min.js?v13') }}
    {{ Hooks::renderScripts() }}

    <script>
        app.perm(<?php echo Helpers::hasSuperAccess() ?>);
        app.user = <?php echo (Sentry::getUser() ?  Sentry::getUser() : 0); ?>;
        ko.applyBindings(app.viewModels.autocomplete, $('.search-bar')[0]);
    </script>
