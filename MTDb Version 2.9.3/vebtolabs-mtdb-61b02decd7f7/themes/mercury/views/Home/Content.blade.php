<section class="content" data-bind="playVideos">
    ...
    <div class="container page-content {{ $slides->isEmpty() ? 'no-slider' : '' }}">
							...
							@if ($category->actor && ! $category->actor->isEmpty())
								@foreach($category->actor->slice(0, $category->limit) as $actor)
									<figure class="col-lg-2 col-md-3 col-sm-4 pretty-figure">
										...
										<figcaption class="no-rating"><a href="{{ Helpers::url($actor->name, $actor->id, 'people') }}">{{ $actor->name }}</a></figcaption>
									</figure>
								@endforeach
							@endif
                                        ...
                                        <figure class="col-lg-2 col-md-3 col-sm-4 pretty-figure">
                                            <div class="{{ $category->show_trailer ? 'play' : '' }}" data-source="{{ $title->trailer }}" data-poster="{{ $title->background }}">
                                                @if ($category->show_trailer && $title->trailer)
                                                    <img data-lazy="{{ $title->poster }}" alt="{{ $title->title }}" class="img-responsive">
                                                    <div class="center"><img src="{{ asset('assets/images/play.png') }}"> </div>
                                                @else
                                                    <a href="{{ Helpers::url($title->title, $title->id, $title->type) }}"><img data-lazy="{{ $title->poster }}" alt="{{ $title->title }}" class="img-responsive"></a>
                                                @endif
                                            </div>
                                            @if(Hooks::hasReplace('Home.ForEachMovie'))
                                                {{ Hooks::renderReplace('Home.ForEachMovie', $title) }}
                                            @endif
                                            <figcaption {{ ! $category->show_rating || ! $title->mc_user_score ? 'class="no-rating"' : ''}}>
		...
		<!-- latest news-->
		@if ($options->enableNewsInHomepage() && ! $options->disableNews())
			<div id="news-container" class="row">
				<h2 class="heading">{{ trans('main.latest news') }}</h2>
					...
					@foreach($news as $k => $item)
						...
						<li>
							<div class="news-item col-sm-3">
								<a href="{{ Helpers::url($item->title, $item->id, 'news') }}"><img data-original="{{ $item->image }}" alt="{{ $item->title }}" class="img-responsive"></a>
                                <div class="title"><a href="{{ Helpers::url($item->title, $item->id, 'news') }}">{{ $item->title }}</a></div>
							</div>
						</li>
					@endforeach
				</ul>
